...
class AppointmentsController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        //inclusive tax minus from service amount
    ...
    public function updateStatus($id, Request $request)
    {
        ...
        if ($status == 'cancelled') {
            ...
            if ($advance_paid_amount && optional($appointment->appointmenttransaction)->payment_status == 0) {
                $user_wallet->amount = $wallet_amount + $advance_paid_amount;
                $user_wallet->update();
            } else {
                $user_wallet->amount = $wallet_amount + $appointment->total_amount;
                $user_wallet->update();
            }

            $wallethistory = new WalletHistory;
            $wallethistory->user_id = $user_wallet->user_id;
            $wallethistory->datetime = Carbon::now();
            $wallethistory->activity_type = 'wallet_refund';
            $wallethistory->activity_message = trans('messages.wallet_refund', ['value' => $appointment->id]);

            $activity_data = [
                'title' => $user_wallet->title,
                'user_id' => $user_wallet->user_id,
                'amount' => $user_wallet->amount,
                'credit_debit_amount' => $advance_paid_amount,
