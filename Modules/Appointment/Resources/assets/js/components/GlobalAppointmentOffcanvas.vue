        ...
        <div class="p-3 bg-primary-subtle border border-primary rounded">
          <div class="d-flex justify-content-between align-items-center mb-2">
            <span class="fw-normal">
              ...
              <button v-if="includedTax.length > 0" data-bs-toggle="modal" data-bs-target="#exampleModalCenter" class="btn btn-link ms-2 p-0">(Included Tax)</button>
              :
            </span>
          ...
          <div class="d-flex justify-content-between align-items-center mb-2">
            <div>
              <span class="fw-normal">{{ $t('appointment.total_tax') }} <i class="ph ph-info btn btn-link ms-2 p-0" data-bs-toggle="modal" data-bs-target="#exampleModalCenter1"></i> :</span>
...
const includedTax = ref([])
const apliedTax = ref([])
const inclusive_tax_amount = ref(0)
const taxCalculation = (amount) => {
  includedTax.value = []
  apliedTax.value = []
  let tax_amount = 0
  totalTaxAmount.value = 0
  inclusive_tax_amount.value = 0
  serviceAmount.value = 0
  console.log('amount', amount)
  serviceAmount.value = amount
  console.log(' serviceAmount.value ', serviceAmount.value)
  if (tax_data.value) {
    tax_data.value.forEach((item) => {
      ...
      if (item.type === 'fixed') {
        if (item.tax_type === 'inclusive') {
          inclusive_tax_amount.value += taxValue
          if (taxValue < serviceAmount.value) {
        ...
        if (item.tax_type === 'inclusive') {
          ...
          inclusive_tax_amount.value += temp_tax_amount
          if (temp_tax_amount < serviceAmount.value) {
  ...
  serviceAmount.value = amount - inclusive_tax_amount.value
  console.log('tax_amount', tax_amount)
  totalTaxAmount.value = tax_amount + inclusive_tax_amount.value
  const total_amount = serviceAmount.value + totalTaxAmount.value
  console.log(apliedTax.value)
  return total_amount
}
